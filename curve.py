...
class Curve:
    ...
    def computeCurve(self):
        ...
        # if only 1 anchor, quadratic bezier curve
            ...
            # calculate the points by lineat interpolation
            for i in range(1, n):
                x[i] = (i * sgm2[0][i] + (n + 1 - i) * sgm1[0][i]) / float(n + 1)
                y[i] = (i * sgm2[1][i] + (n + 1 - i) * sgm1[1][i]) / float(n + 1)

            # save the points in format [x1,y1,x2,y2,...]
            points = (x, y)
        ...
        # if 2 anchors cubic bezier curve
            ...
            # calculate curve by linear interpolation
            for i in range(1, n):
                line1x1[i] = sgm1[0][i]
                line1y1[i] = sgm1[1][i]
                line1x2[i] = sgm2[0][i]
    ...
    def drawCurve(self, meta):
        ...
        self.me = meta.canvas.create_line(self.locus, tags='curve', fill=meta.get_color(),
                                          width=meta.get_width())  # curve path
    ...
    def drawAnchor1(self, meta):
        if self.anchor1x != None:
            ...
            self.anchor1L = meta.canvas.create_line(self.x1, self.y1, self.anchor1x, self.anchor1y, tags='anchor_line',
                                                    fill='#ff00ff')
            self.anchor1H = meta.canvas.create_oval(self.anchor1x - 3, self.anchor1y - 3, self.anchor1x + 3,
                                                    self.anchor1y + 3,
    ...
    def drawAnchor2(self, meta):
        if self.anchor2x is not None:
            ...
            self.anchor2L = meta.canvas.create_line(self.x2, self.y2, self.anchor2x, self.anchor2y, tags='anchor_line',
                                                    fill='#ff00ff')
            self.anchor2H = meta.canvas.create_oval(self.anchor2x - 3, self.anchor2y - 3, self.anchor2x + 3,
                                                    self.anchor2y + 3,
                                                    fill='blue', activefill='red', tags='anchor')
...
class HelperObject:
    def __init__(self,meta):
        ...
        self.state = 0  # stages of drawing
        self.curves = []  # keep track of all curve sections
        self.object = None  # current section of curve
        self.meta = meta

    # deactivates the curve
    def clear(self, c):
        self.state = 0
        if self.curves:
            c.delete(self.curves[0].point1)
        for curve in self.curves:
            curve.deleteCurve(c)


# control click events during drawing

# control pressing mouse
def masterClick(x, y, helperObject, meta):
    # before any press
    if helperObject.state == 0:
        # start a new curve and set initail points
    ...
    # after drawing finished
        ...
        # if clicked on no object, restart tool to draw another curve
...
def masterRelease(x, y, helperObject, meta):
    ...
    # next clicks set curve points and draw
...
def masterPressedMotion(x, y, helperObject, meta):
    ...
    # subsequent times, update anchor 2 and redraw curve section
...
def masterMotion(x, y, helperObject, meta):
    ...
    # after curve is drawn change to curve editing mode
...
# binds canvas to an anchor during curve editing
# because the mouse moves faster than the item can catch
...
# binds canvas to a point during curve editing
# because the mouse moves faster than the item can catch
...
# one point can belong to 2 curves
def movePoint(id, e, helperObject, meta):
    ref = None # to link to curves
    for i in helperObject.curves:
        if i.point2 == id: # find first curve of selected point
        ...
        elif i.point1 == id: # find second curve
            # distance mouse moved
            deltax = e.x - i.x1
            deltay = e.y - i.y1
            i.setPoint1(e.x, e.y)
            ...
            i.drawCurve(meta)
            if ref is not None: # if another curve section has the same point
                i.point1 = ref # link to the same item
            else:
                i.drawPoint1(meta)
...
# unbinds canvas from items when mouse released and rebinds points and anchors
...
def restart(helperObject, meta):
    ...
    # draw the fianalized curve to the image
    for i in helperObject.curves:
        draw.line(i.locus, fill=meta.get_color(), width=meta.get_width())
        i.deleteCurve(meta.canvas)

    # push edited image to canvas
    meta.draw(image)

    # delete canvas items (anchors, points and lines)
    meta.canvas.delete(helperObject.curves[0].point1)
    helperObject.curves = []

    # reset helper object fields
...
def activate(meta):
    helperObject = HelperObject(meta) # create a helper object
    meta.canvas.bind('<Button-1>', lambda e: masterClick(e.x, e.y, helperObject, meta))
    meta.canvas.bind('<B1-Motion>', lambda e: masterPressedMotion(e.x, e.y, helperObject, meta))
    meta.canvas.bind('<ButtonRelease-1>', lambda e: masterRelease(e.x, e.y, helperObject, meta))
    meta.canvas.bind('<Motion>', lambda e: masterMotion(e.x, e.y, helperObject, meta))
